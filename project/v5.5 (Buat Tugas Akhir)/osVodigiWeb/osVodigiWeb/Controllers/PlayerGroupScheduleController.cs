    ...
    public class PlayerGroupScheduleController : Controller
    {
        ...
        private string BuildScheduleTable(int playergroupid)
        {
            try
            {
                ...
                sb.AppendLine("   <tr>");
                sb.AppendLine("      <td class=\"gridheader\" colspan=\"8\" style=\"text-align:right\"><span id=\"clearlink\"><a href=\"" + uri + "?deleteall=1\">Clear Schedule</a></span></td>");
                sb.AppendLine("   </tr>");
                sb.AppendLine("   <tr>");
        ...
        private string GetSchedulesByDayHour(List<PlayerGroupSchedule> schedules, int day, int hour)
        {
            try
            {
                ...
                foreach (PlayerGroupSchedule schedule in schedules)
                {
                    ...
                    {
                        Screen screen = screenrep.GetScreen(schedule.ScreenID);
                        if (screen != null)
                        {
